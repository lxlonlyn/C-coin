...
from PyQt5.QtWidgets import QApplication, QWidget, QListWidget, QStackedWidget, QFormLayout, QLineEdit, QPushButton, \
    QHBoxLayout, QVBoxLayout, QTableWidget, QTabWidget, QMainWindow, QMessageBox, QRadioButton, QLabel, QCheckBox, \
    QFrame, QScrollArea, QInputDialog
import BlockWidget
from blockchain import Blockchain
from block import Block
from main import createUser, User, getUTXO, makeDeal
...
class MainWindow(QTabWidget):
    ...
    def func(self):
        ...
        print(self.all_user[2].addr)
        print(self.all_user[1].wif)

        # 创建区块链
        # 首先创建创世区块，矿工为 bright
        foundation = Block(self.all_user[0].addr)
        second_block = Block(self.all_user[1].addr)
        # 创建区块链
        blockchain = Blockchain()
        blockchain.addBlock(foundation)
        blockchain.addBlock(second_block)
    ...
    def create_block_clicked(self):
        ...
        for i in range(len(self.all_user)):
            if self.all_user[i].addr == input_addr:
                found = i
                break
        print(found)
        if okPressed:
    ...
    def deal_clicked(self):
        ...
        to_pos = -1
        for i in range(len(self.all_user)):
            if self.all_user[i].addr == self.le_addr.text():
                to_pos = i
                break
